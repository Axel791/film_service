...
class PersonService:
    ...
    async def get(self, person_id: str) -> Person:
        person = await self.cache.get(key=person_id, schema=Person)
        if person is None:
            person = await self.storage.get(
                obj_id=film_id, index="persons", schema=Person
            )
    ...
    async def search(
        ...
        return await self._search_persons(query=query, page=page, page_size=page_size)
    ...
    async def _get_films_by_person_id(
        ...
        body = {"query": {"terms": {"id": film_ids}}, "from": start, "size": page_size}
        ...
        return await self._return_persons(
            body=body, schema=FilmWorkShort, index="movies"
        )
    ...
    async def _search_persons(
        ...
        return await self._return_persons(body=body, schema=Person, index="persons")

    async def _return_persons(
        self, body: dict, index: str, schema: Type[BaseModel]
    ) -> Type[BaseModel] | None:
        key = json.dumps(body)
        persons = await self.cache.list(key=key, schema=schema)
        if persons is None:
            persons = await self.storage.list(
                key=key, body=body, schema=schema, insex=index
            )
