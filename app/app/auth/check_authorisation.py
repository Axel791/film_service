...
def check_authorisation(token: str) -> CheckToken:
    ...
    try:
        response: requests.Response = requests.get(url=settings.authentication_url, headers=header)
        if response.status_code != http.HTTPStatus.OK:
            return CheckToken(is_valid=False, message=f'Request error: {response.status_code}')
