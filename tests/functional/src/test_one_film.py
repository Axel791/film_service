...
@pytest.mark.parametrize(
    ...
    [
        (CORRECT_FILM_UUID, {"status": 200, "film_id": CORRECT_FILM_UUID}),
        (WRONG_FILM_UUID, {"status": 404, "film_id": None}),
    ],
)
@pytest.mark.asyncio
async def test_one_film(es_write_data, make_get_request, query_data, expected_answer):
    ...
    response: dict = await make_get_request(url=f"/api/v1/films/get/{query_data}")
