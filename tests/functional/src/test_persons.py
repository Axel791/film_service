...
from tests.conftest import generate_persons, generate_film
...
@pytest.mark.parametrize(
    ...
    [
        (
            CORRECT_PERSON_UUID,
            {'status': 200, 'person_id': CORRECT_PERSON_UUID}
        ),
        (
            WRONG_PERSON_UUID,
            {'status': 404, 'person_id': WRONG_PERSON_UUID}
        )
    ]
)
@pytest.mark.asyncio
async def test_one_person(
    ...
    response: dict = await make_get_request(
        url=f'/api/v1/persons/get/{query_data}'
    )
