...
def generate_persons(person_id: str | None = None, film: dict | None = None) -> dict:
    person = {
        "id": str(uuid.uuid4()) if person_id is None else person_id,
        "full_name": "Ann",
        "films": [film]
        if film is not None
        else [generate_film(film_id=str(uuid.uuid4()))],
    }
    return person
...
def get_es_bulk_query(
    es_data: List[dict], es_index: str, es_id_field: str
) -> List[str]:
    bulk_query = []
    for row in es_data:
        bulk_query.extend(
